...
class CpsProject extends FileSystemBasedProject_1.FileSystemBasedProject {
    ...
    parseDocument(document) {
        ...
        if (!this.document.Project.ItemGroup)
            return;
        this.document.Project.ItemGroup.forEach(items => {
            if (items.PackageReference) {
                items.PackageReference.forEach(pack => {
                    this.packages.push(new PackageReference_1.PackageReference(pack.$.Include, pack.$.Version));
                });
            }
            if (items.ProjectReference) {
                items.ProjectReference.forEach(reference => {
                    ...
                    this.references.push(new ProjectReference_1.ProjectReference(ref, reference.$.Include));
                });
            }
        });
    }
